...
namespace
{
     ...
     * currently valid -- deliberately NOT a second kAtomNumPoints-sized pair of buffers:
    ...
    // font.h's drawTextScaled() instead. `caption` (the fuller "Fe (2/5)" line -- element
    // symbol, not the shell notation already shown by bigLabel) stays plain-size underneath
    // it -- scaling THAT up too would overflow the 240px width.
     ...
     * electron count ("<occ>e-") as a note in the top-right corner -- shared by the eased
     ...
     * `frameSalt`/`buzzThreshold` feed renderScene()'s hidden-points buzz (see camera.h's
     * kHiddenPointsFraction) -- caller owns the per-call frameSalt counter since this
     * function doesn't loop itself. */
