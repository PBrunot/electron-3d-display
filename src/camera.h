...
#include <cstdint>
#include <cstring> // renderScene()'s std::memset, a template so it must stay in this header
 ...
 * both AtomPoint and OrbitalPoint qualify) into `frameBuf`, all in a single `color`.
 * frameBuf is NOT cleared by this function -- caller clears first. Convenience wrapper for
 * a still-uncolored cloud (used by main.cpp's ATOM_VIEW_TEST build); per-point coloring
...
/**
 * Render `count` points with PER-POINT colors (colors[i] for points[i]), plus an optional
 * "buzz" flicker: a point is skipped this frame if a cheap per-point/per-frame hash falls
 * below buzzThreshold (0 -- the default -- disables buzz entirely: since the hash is
 * unsigned, "hash < 0" is never true, so every point always draws). Port of
 * device_render_common.py's render_points() viper kernel, minus the Q8 fixed-point (see
 * this file's header comment) and using the same multiplicative hash constants
 * (668265261/374761393 -- Bob Jenkins'/xxHash's 32-bit constants) on the point index and
 * a per-frame salt.
 */
template <typename PointT>
...
/**
 * Clear, draw the proton marker, then every point at the given camera pose/scale. Does
 * NOT draw title/FPS/scale-bar text or present the frame -- caller does both, matching
 * device_render_common.py's render_frame()/run()-loop division of labor (both its
 * fly_over() and each viewer's steady-state loop call render_frame() then separately
 * layer text on top).
 */
template <typename PointT>
...
constexpr orb_real_t kZoomExcursionScaleMaxFactor = orb_real_t(5.0);
constexpr int kZoomExcursionEaseFrames = 30;
