...
// number n, valence subshell brightened / core dimmed), calibrated scale (see
// atom_cloud.h's pixelsPerBohrForCanvas()/scaleForAtom()), per-shell-colored wrapped
// electron-configuration title. Port of micropython/atom_view.py, built on
// atom_cloud.h's model layer and camera.h's render/fly-over pipeline -- see orbital_view.h
// for the sibling hydrogen-orbital viewer this mirrors.
//
...
constexpr int kAtomViewDefaultZ = 6; // carbon -- simplest element with a non-full, non-empty p subshell
...
struct AtomPresetState {
    ...
    /** Build this element's point cloud, shell colors, and calibrated scale.
     * `pixelsPerBohr` is pixelsPerBohrForCanvas()'s result -- computed once by the caller
     * and reused across elements, not recomputed per load(). */
    void load(int zIn, orb_real_t pixelsPerBohr);
};
