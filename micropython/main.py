...
examples/corner_calibration.ino applies for the C++ port). Confirms SPI
wiring, the panel-mirror fix, BGR color order, and the 180 degree offset
before any point-cloud rendering is built on top (CLAUDE.md M1).

On this board, `mpremote run main.py` hung after a watchdog reset (looked
like a raw-paste-mode / USB-passthrough interaction, not a MicroPython or
driver bug). What worked reliably instead:
    mpremote connect <port> fs cp st7789py.py display.py main.py :
...
def main():
    ...
    print(
        ...
        "the 180 degree offset in display.py/main.py no longer matches this "
        "unit's mount -- see examples/corner_calibration/README.md."
    )
